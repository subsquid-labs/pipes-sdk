# RFC: Declarative Streams API for EVM Portal Sources
...
This RFC proposes a **declarative streams API** for `evmPortalSource` in Pipes SDK.
...
* Removing or breaking existing transformers (backward compatibility is expected)
...
const source = evmPortalSource({
  ...
  outputs: {
    ...
    // RPC latency metrics
    latency: evmRpcLatencyProfiler(),
  },
})

await source
  .pipe(({ raw, transfers, latency }, ctx) => {
...
```ts
raw: evmQueryFragment().addRange({ from: 'latest' })
```

Declares raw data output without transformation.
...
```ts
transfers: evmDecoder(...)
  .transform(normalizeTransfers)
  .transform(enrichWithMetadata)
```

Streams may be defined using decoders or transformer pipelines.
Only transformers that explicitly declare query requirements contribute to query construction.
Additional `.transform(...)` steps are applied as post-processing and do not affect query shape unless explicitly declared.
...
* Naming:
    ...
    * `streams` vs `pipelines` vs `outputs`
    * `Decoder` vs `Transformer` vs `QueryAwareTransformer`
    * `evmQueryFragment()` vs `EvmQueryBuilder` vs `EvmQueryFragment`
    * `.transform()` vs `.pipe()`

...
import { clickhouseDefaults } from '../../project-files/dynamic/docker-compose.js'
...
export class ClickhouseSinkBuilder extends BaseSinkBuilder {
  render() {
    const templates = this.config.templates
      ...
      .map((t) => ({
        ...t,
        tableNames: this.extactTableNames(t.renderFns.clickhouseTables()),
      }))
  ...
  async createMigrations(): Promise<void> {
    this.config.templates
      .forEach((t) => {
        this.projectWriter.createFile(`migrations/${t.templateId}-migration.sql`, t.renderFns.clickhouseTables())
      })
  }

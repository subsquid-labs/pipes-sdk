...
export function renderSchemasTemplate<N extends NetworkType>(config: Config<N>): string {
  const renderedSchemas = config.templates.map((t) => t.renderFns.postgresSchemas())
  const schemaImports = renderedSchemas.flatMap((schema) => splitImportsAndCode(schema).imports)

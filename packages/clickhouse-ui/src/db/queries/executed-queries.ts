...
export async function fetchRecentQueries(window: QueryLogWindow = DEFAULT_QUERY_LOG_WINDOW): Promise<QueryLogRow[]> {
...
export async function fetchLatestQueryByHash(queryHash: string): Promise<QueryDetails | null> {
  const client = getClickHouseClient()
  const resultSet = await client.query({
    query: `
WITH
...
export async function fetchExplainPlan(sql: string): Promise<string> {
  const client = getClickHouseClient()
  const resultSet = await client.query({
...
export async function fetchExplainPipeline(sql: string): Promise<string> {
  const client = getClickHouseClient()
  const resultSet = await client.query({
